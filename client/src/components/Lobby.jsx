...
export default function Lobby() {
  ...
  const socket = useSocket({
    ...
    'lobby-chat-history': (msgs) => setChatMessages(msgs),
    'lobby-chat': (msg) => setChatMessages(prev => [...prev, msg]),
  })
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-4 py-12">
      <div className="w-full max-w-4xl flex flex-col lg:flex-row gap-6 items-start justify-center">
        ...
        <div className="w-full lg:w-80 flex-shrink-0 bg-gray-900 rounded-2xl border border-gray-800 shadow-xl flex flex-col h-[600px] lg:sticky lg:top-12">
          <div className="px-4 py-3 border-b border-gray-800">
            <h2 className="text-sm font-bold text-gray-400 uppercase tracking-wider">💬 로비 채팅</h2>
          </div>
          <div className="flex-1 overflow-y-auto p-3 space-y-2">
            {chatMessages.length === 0 && (
              <p className="text-gray-600 text-xs text-center mt-4">아직 메시지가 없어요. 먼저 인사해봐요!</p>
            )}
          ...
          <form onSubmit={handleChatSend} className="p-3 border-t border-gray-800 flex gap-2">
            <input
              className="flex-1 bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-yellow-400 transition disabled:opacity-40"
              placeholder={nickname.trim() ? '메시지 입력...' : '닉네임을 먼저 입력하세요'}
              value={chatInput}
            ...
            <button
              type="submit"
              disabled={!nickname.trim() || !chatInput.trim()}
              className="bg-yellow-400 hover:bg-yellow-300 text-black font-bold px-3 py-2 rounded-lg text-sm transition disabled:opacity-40 disabled:cursor-not-allowed"
            >
              전송
            </button>
          </form>
